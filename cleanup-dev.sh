#!/usr/bin/env bash
#
# cleanup-dev.sh -- undo whatever publish-dev.sh left behind, and retire
# previews that are no longer wanted.
#
#     ./cleanup-dev.sh                    # local reset: stale profiles + report
#     ./cleanup-dev.sh --outputs          # ...and delete _book-dev/
#     ./cleanup-dev.sh --clone            # ...and delete the staging clone
#     ./cleanup-dev.sh --all              # everything local
#     ./cleanup-dev.sh --list             # list published previews
#     ./cleanup-dev.sh --unpublish pr-123 # remove one preview from the site
#
#
# WHEN YOU NEED THIS
# ------------------
# publish-dev.sh has an EXIT trap that removes its temporary profile on Ctrl-C
...
# because publish-dev.sh re-renders the output directory and resets the clone
...
while (( $# > 0 )); do
  case "$1" in
    ...
    --unpublish)
      ...
      if (( $# < 2 )); then
        echo "cleanup-dev.sh: --unpublish needs a slug" >&2
        exit 1
      fi
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "cleanup-dev.sh: unknown option '$1' (try --help)" >&2
      exit 1
      ;;
  esac
  shift
...
if [[ -n $unpublish_slug ]]; then
  ...
  if [[ ! -d $DEV_CLONE_DIR/$unpublish_slug ]]; then
    echo "cleanup-dev.sh: no preview named '$unpublish_slug' is published" >&2
    echo "  (./cleanup-dev.sh --list shows the ones that are)" >&2
    exit 1
  fi
  ...
  dev_commit_and_push "remove dev preview: $unpublish_slug"
...
if [[ $do_clone == true ]]; then
  ...
  # publish-dev.sh will silently discard when it resets onto the remote.
  if git -C "$DEV_CLONE_DIR" rev-parse --verify --quiet HEAD >/dev/null 2>&1; then
    ...
    if [[ $unpushed != "0" ]]; then
      ...
      echo "          the next ./publish-dev.sh will discard them"
    fi
  fi
